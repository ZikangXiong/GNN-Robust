...
class AndProp(AbsProp):
    ...
    def rule(self, device=None) -> Tensor:
    ...
    def props_of(self, rule: Tensor) -> List[AbsProp]:
        """ Return the corresponding properties of certain indices. """
        idxs = rule.nonzero().squeeze(dim=-1)
    ...
    def safe_dist(self, outs: AbsEle, rules: Tensor, *args, **kwargs):
        ...
        for i, prop in enumerate(self.props):
            ...
            full_dists = torch.zeros(len(rules), *piece_dists.size()[1:], device=piece_dists.device)
    ...
    def viol_dist(self, outs: AbsEle, rules: Tensor, *args, **kwargs):
        ...
        for i, prop in enumerate(self.props):
            ...
            full_dists = torch.full((len(rules), *piece_dists.size()[1:]), float('inf'), device=piece_dists.device)
            full_dists.scatter_(0, bits, piece_dists)
            res.append(full_dists)
